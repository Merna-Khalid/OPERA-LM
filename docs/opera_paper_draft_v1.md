...
2. **A matched-protocol characterization** against RoPE and NoPE transformers whose data pipeline, loss, evaluation, and seeding are imported from OPERA's own code (Section 3), at matched parameters, on both sentence-level (T≤20) and document-level (T≤256, evaluated to 2× beyond) regimes.
3. **The position-curve and interrogation-probe instruments**, applied identically across architectures, yielding (a) the flattest length-degradation profile for OPERA among the mechanisms tested and (b) a cross-architecture finding: early-context forgetting is learned, not structural.
...
**Pre-registration.** For each architectural hypothesis we recorded, in the code shipped before the run, the falsifiable prediction and the decision rule, including (for the rotation constraint) a one-rematch rule binding in both directions. Section 5 reports outcomes against those rules verbatim.
...
**Recipe asymmetry.** Both architectures share one training recipe (lr, warmup, schedule) whose defaults encode transformer folklore; OPERA is untuned. The comparison is therefore conservative against OPERA by an unknown margin.

## 8. Future work

Motivated directly by the measurements above: a multi-state readout (attention over the fold's running accumulator states, with the same deterministic level encoding — extrapolation-safe by construction) targeting the mid-band capacity gap; objective-side pressure on early context (multi-scale node supervision, retrieval-style auxiliaries), targeting the learned forgetting that Section 4.4 shows no architecture escapes; the exit-normalized rack fold of Section 5.3; the crossover timing measurement; a subword-tokenized protocol enabling literature-comparable numbers and removing the `<unk>` confound; fused composition kernels (a Metal prototype exists; a CUDA/Triton port with the verified adjoint as reference is the direct path to closing the wall-clock gap); and parameter-axis scaling, depth-first with tied embeddings.

## 9. Reproducibility

Every model, evaluation, probe, and equivalence claim in this paper is exercised by self-tests shipped in the released code (including numerical verification of the rack axiom, exact-equivalence asserts for fold compaction, causality asserts for every fold and PE variant, and gradient checks for the custom composition backward). Data splits are internally seeded and byte-identical across architectures; variation seeds are applied after data loading; training checkpoints save full RNG state for exact-stream resumption. All experiments were run on single consumer/Colab-class GPUs; the largest single run is ~3.5 GPU-hours.

## Acknowledgments

Engineering and drafting assistance from Claude (Anthropic). All experimental decisions, runs, and the research direction are the author's. [TODO: confirm/edit disclosure wording to taste.]
